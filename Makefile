...
LIBVER_GEN := $(GEN_DIR)/cjelly/libver_gen.h
...
$(LIBVER_GEN): Makefile
	...
	@printf '%s\n' \
		...
		'#ifndef CJELLY_LIBVER_GEN_H' \
		'#define CJELLY_LIBVER_GEN_H' \
		'' \
		'/** The symbol namespace for this build, from the Makefile'"'"'s BRANCH. */' \
		'#define CJELLY_NS_NAME $(LIBVER_SYMBOL)' \
		'' \
		'#endif // CJELLY_LIBVER_GEN_H' > $@
...
ifeq ($(OS_NAME), Linux)
	...
	if [ -n "$$leaked" ]; then \
		...
		printf "%s\n" "$$leaked" >&2; \
		printf "\nEach needs a '#define <name> CJ_NS(<name>)' line in namespace.h, or\n" >&2; \
